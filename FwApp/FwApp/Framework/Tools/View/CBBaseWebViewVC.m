...
static void *WKWebViewBrowserContext = &WKWebViewBrowserContext;
...
-(void)setHideAllBtn:(BOOL)hideAllBtn
{
    _hideAllBtn = hideAllBtn;
    if (hideAllBtn==YES) {
...
//    [self observeWeChatByWebView:webView decidePolicyForNavigationAction:navigationAction decisionHandler:decisionHandler];
...
- (WKWebView *)wkWebView{
    if (!_wkWebView) {
        ...
        preferences.minimumFontSize = 13; // 最小字号
...
//        _wkWebView.scrollView.bounces = NO;   ///< 允许滚动
        ...
        //kvo 添加进度监控
        [_wkWebView addObserver:self forKeyPath:NSStringFromSelector(@selector(estimatedProgress)) options:0 context:WKWebViewBrowserContext];
...
// 监听微信
- (void)observeWeChatByWebView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    WKNavigationActionPolicy actionPolicy = WKNavigationActionPolicyAllow;
    NSString *urlString = [[navigationAction.request URL] absoluteString];
//    urlString = [urlString stringByRemovingPercentEncoding:NSUTF8StringEncoding];
